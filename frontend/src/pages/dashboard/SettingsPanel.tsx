...
export default function SettingsPanel({
  ...
  return (
    <div className="rounded-2xl border border-border bg-card/95 p-6 shadow-sm">
      <h2 className="text-xl font-semibold mb-6 flex items-center gap-2"><Settings className="h-5 w-5" /> {biLabel('Service Configuration', '系统配置')}</h2>
      ...
      <div className="space-y-6 max-w-3xl">
        ...
        <div className="rounded-xl border border-border/60 bg-background/40 p-4 space-y-4">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-start">
            <div className="flex-1 grid gap-2">
              ...
              />
              <p className="text-[11px] text-muted-foreground italic">{biLabel('Domains are saved back into ', '域名会保存回 ')}<code>mail_email_domains</code>{biLabel(' in config.yaml.', ' 配置项。')}</p>
            </div>
            <button
              type="button"
              onClick={handleAddDomain}
              className="inline-flex h-10 min-w-28 items-center justify-center rounded-md border border-border bg-background px-4 py-2 text-sm font-medium hover:bg-muted transition-colors"
            >
              {biLabel('Add Domain', '添加域名')}
            </button>
          </div>
          {domainEditorError && (
            <div className="text-sm px-4 py-3 rounded-md border border-destructive/50 bg-destructive/10 text-destructive">
              {domainEditorError}
            </div>
          )}
                      ...
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div className="flex flex-wrap gap-2">
                          <button
                            ...
                            className="inline-flex h-9 items-center justify-center rounded-md border border-border bg-background px-3 text-sm font-medium hover:bg-muted transition-colors"
                          >
                          ...
                          <button
                            ...
                            disabled={isTesting}
                            className="inline-flex h-9 items-center justify-center rounded-md border border-border bg-background px-3 text-sm font-medium hover:bg-muted disabled:opacity-50 transition-colors"
                          >
                          ...
                          <button
                            ...
                            className="inline-flex h-9 items-center justify-center rounded-md border border-destructive/40 bg-destructive/5 px-3 text-sm font-medium text-destructive hover:bg-destructive/10 transition-colors"
                          >
                            {biLabel('Remove', '移除')}
                          </button>
                        </div>
        ...
        <div className="space-y-4 pt-6 border-t border-border mt-6">
          ...
          <div className="p-3 bg-muted/30 rounded-lg text-xs text-muted-foreground space-y-2 border border-border/50 mb-4">
          ...
          {codexReplenishEnabled && (
            <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="grid gap-2">
                ...
                <input
                  type="number"
                  value={codexReplenishTargetCount}
                  onChange={(e) => setCodexReplenishTargetCount(parseInt(e.target.value) || 0)}
              ...
              <div className="grid gap-2">
                ...
                <input
                  type="number"
                  value={codexReplenishThreshold}
                  onChange={(e) => setCodexReplenishThreshold(parseInt(e.target.value) || 0)}
                  className="flex h-10 w-full rounded-md border border-input bg-background/50 px-3 py-2 text-sm"
                  min="0"
                />
              </div>
        ...
        <div className="space-y-4 pt-6 border-t border-border mt-6">
          ...
          <button
            ...
            className="inline-flex h-10 items-center justify-center rounded-md border border-border bg-background px-4 py-2 text-sm font-medium hover:bg-muted disabled:opacity-50 transition-colors"
          >
          ...
          {remoteTestResult && (
            <div className="text-sm px-4 py-3 rounded-md border border-border bg-background/70 space-y-2">
