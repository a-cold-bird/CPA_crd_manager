...
PROXY_RETRY_ATTEMPTS_PER_ACCOUNT = max(1, int(_CONFIG.get("proxy_retry_attempts_per_account", 3)))
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                if done == total or (now - last_log_at) >= 1.5:
                    with _print_lock:
                        print(f"[ProxyCheck] progress={done}/{total}, valid={len(valid)}")
                    last_log_at = now

        elapsed = time.time() - started_at
        with _print_lock:
            print(f"[ProxyCheck] done: valid={len(valid)}/{total}, elapsed={elapsed:.1f}s")
        return valid
...
def fetch_sentinel_challenge(session, device_id, flow="authorize_continue", user_agent=None,
                             sec_ch_ua=None, impersonate=None, error_sink=None):
    ...
    attempts = []
    if debug_proxy:
        attempts.append({"verify": False, "proxies": {"http": debug_proxy, "https": debug_proxy}})
    attempts.append({})

    last_error = None
    for extra in attempts:
        try:
            resp = session.post("https://sentinel.openai.com/backend-api/sentinel/req", **kwargs, **extra)
        except Exception as exc:
            last_error = exc
            continue
        ...
        if resp.status_code != 200:
            if isinstance(error_sink, dict):
                error_sink.update({
                    ...
                    "message": clean_display_text(resp.text[:500]) or f"Sentinel challenge failed with HTTP {resp.status_code}",
    ...
    if isinstance(error_sink, dict):
        error_sink.update({
            ...
            "message": clean_display_text(str(last_error or "Sentinel challenge request failed"))[:500],
        })
    return None
...
from .email_utils import create_test_email, fetch_verification_code, snapshot_mailbox_ids, snapshot_mailbox_max_id

def create_temp_email(provider="mailfree", domain=None):
    """Doc."""
    email, mailbox = create_test_email(provider=provider, domain=domain)
    if email:
...
def create_otp_wait_context(mail_token: str):
    return {
        "started_at": time.time(),
        "seen_email_ids": snapshot_mailbox_ids(mail_token) if mail_token else set(),
        "baseline_email_id": snapshot_mailbox_max_id(mail_token) if mail_token else None,
        "tried_codes": set(),
    }


def wait_for_verification_email(mail_token: str, timeout: int = 120, otp_context=None, exclude_codes=None):
    """Doc."""
    context = otp_context or create_otp_wait_context(mail_token)
    max_wait = max(1, int(timeout))
    return fetch_verification_code(
        mail_token,
        max_wait=max_wait,
        interval=2,
...
class ChatGPTRegister:
    ...
    def __init__(
        ...
        if self.proxy:
            ...
            # curl_cffi will still honor HTTP_PROXY/HTTPS_PROXY from the environment
            # unless we explicitly blank them out on the session.
            self.session.proxies = dict(DIRECT_PROXIES)
    ...
    def _log(self, step, method, url, status, body=None):
        ...
        if body:
            ...
            try:
                lines.append(f"{prefix}[Response] {json.dumps(body, indent=2, ensure_ascii=False)[:response_limit]}")
            except Exception:
    ...
    def create_temp_email(self):
        email, mail_token = create_test_email(provider=self.email_provider, domain=self.email_domain)
        if not email:
            raise Exception("email_utils temporary email creation failed")
        return email, "dummy_pwd", mail_token

    def create_otp_wait_context(self, mail_token: str):
        seen_email_ids = snapshot_mailbox_ids(mail_token) if mail_token else set()
        return {
            "started_at": time.time(),
    ...
    def _extract_error_info(data):
        ...
        error_message = str(error_block.get("message") or data.get("message") or data.get("text") or "").strip()
        return error_code, error_message

    def wait_for_verification_email(self, mail_token: str, timeout: int = 120, otp_context=None, exclude_codes=None):
        """Doc."""
        self._print(f"[OTP] waiting for verification email (timeout={timeout}s)...")
        context = otp_context or self.create_otp_wait_context(mail_token)
        ...
        if code:
            self._print(f"[OTP] received code: {code}")
            return code
        self._print(f"[OTP] waiting for OTP email up to {timeout}s; polling mailbox")
        return None
    ...
    def authorize(self, url: str) -> str:
        ...
        response_headers = {
            ...
            if str(key or "").lower() in {"content-type", "server", "cf-ray", "set-cookie", "location", "cache-control"}
        }
        self._log("3. Authorize", "GET", url, r.status_code, {
            "final_url": final_url,
        ...
        if r.status_code >= 400:
            ...
            if "just a moment" in lowered_excerpt or "enable javascript and cookies to continue" in lowered_excerpt or "_cf_chl_opt" in lowered_excerpt:
    ...
    def callback(self, url: str = None):
        ...
        for i in range(2):
            try:
                ...
                if i == 0:
                    self._print(f"[Callback] transient failure, retrying once: {e}")
                    time.sleep(2)
                else:
                    raise e
    ...
    def run_register(self, email, password, name, birthdate, mail_token):
        """Doc."""
        otp_context = self.create_otp_wait_context(mail_token) if mail_token else None
        self.visit_homepage()
        ...
        self._print(f"[Auth] authorize path: {final_path}")

        need_otp = False

        if "create-account/password" in final_path:
            self._print("[Auth] reached an unsupported auth step")
            ...
            self._print("[OTP] authorize already reached email verification; waiting for OTP")
            ...
            self._print("[OTP] OTP validation failed during authorize flow")
            _random_delay(0.5, 1.0)
            ...
            self._print("[Auth] callback/chatgpt final URL reached")
            return True
        else:
            self._print(f"[Auth] unexpected final URL, falling back to register+otp: {final_url}")
        ...
        if need_otp:
            ...
            while time.time() < otp_deadline:
                ...
                if error_code == "wrong_email_otp_code" or "wrong code" in error_message.lower():
                    self._print("[OTP] wrong_email_otp_code; re-sending OTP and waiting again...")
                    self.send_otp()
            ...
            if status != 200:
                raise Exception(f"OTP validation failed ({status}): {data or 'OTP validation returned no payload'}")
    ...
    def _oauth_allow_redirect_extract_code(self, url: str, referer: str = None):
        ...
        try:
            ...
            if code:
                self._print("[OAuth] allow_redirect extracted code from final URL")
                return code
            ...
            for r in getattr(resp, "history", []) or []:
                ...
                if code:
                    self._print("[OAuth] allow_redirect extracted code from history Location")
                    return code
                code = _extract_code_from_url(str(r.url))
                if code:
                    self._print("[OAuth] allow_redirect extracted code from history URL")
                    return code
        except Exception as e:
            ...
            if maybe_localhost:
                ...
                if code:
                    self._print("[OAuth] allow_redirect extracted code from localhost callback")
                    return code
            self._print(f"[OAuth] allow_redirect failed: {e}")
        ...
        for hop in range(max_hops):
            try:
                ...
                if maybe_localhost:
                    code = _extract_code_from_url(maybe_localhost.group(1))
                    if code:
                        self._print(f"[OAuth] follow[{hop + 1}] captured localhost callback URL")
                        return code, maybe_localhost.group(1)
                self._print(f"[OAuth] follow[{hop + 1}] request failed: {e}")
    ...
    def _oauth_submit_workspace_and_org(self, consent_url: str):
        ...
        if not session_data:
            ...
            self._print(f"[OAuth] missing oai-client-auth-session cookie, cookies={cookie_names[:12]}")
            return None

        workspaces = session_data.get("workspaces", [])
        if not workspaces:
            self._print("[OAuth] session bootstrap returned workspace information")
            return None

        workspace_id = (workspaces[0] or {}).get("id")
        if not workspace_id:
            self._print("[OAuth] workspace_id resolved")
            return None
        ...
        if resp.status_code != 200:
            self._print(f"[OAuth] workspace/select failed: {resp.status_code}")
            return None

        try:
            ws_data = resp.json()
        except Exception:
            self._print("[OAuth] workspace/select returned non-JSON response")
            return None
        ...
        if org_id:
            ...
            if resp_org.status_code == 200:
                try:
                    ...
                    self._print("[OAuth] organization/select returned non-JSON response")
                    return None
    ...
    def perform_codex_oauth_login_http(self, email: str, password: str, mail_token: str = None):
        ...
        def _remember_protocol_error(
            ...
            self.last_protocol_error = {
                ...
                "message": clean_display_text(str(message or "").strip())[:excerpt_limit],
                "stage": stage,
                "status_code": status_code,
                "response_excerpt": clean_display_text(str(response_excerpt or "").strip())[:excerpt_limit],
            }
        ...
        def _response_excerpt(resp) -> str:
            ...
            if isinstance(data, dict):
                ...
                if err_code or err_message:
                    return clean_display_text(" / ".join(part for part in (err_code, err_message) if part))[:excerpt_limit]
                return clean_display_text(json.dumps(data, ensure_ascii=False))[:excerpt_limit]

            return clean_display_text(str(data))[:excerpt_limit]
        self._print("[OAuth] starting Codex OAuth flow...")
        otp_context = self.create_otp_wait_context(mail_token) if mail_token else None
        ...
        def _bootstrap_oauth_session():
            ...
            try:
                ...
                self._print(f"[OAuth] /oauth/authorize request failed: {e}")
                _remember_protocol_error("oauth_bootstrap", code="authorize_request_failed", message=str(e))
                return False, "", False
            ...
            self._print(f"[OAuth] login_session: {'present' if has_login else 'missing'} / cookies={_cookie_names()}")

            if not has_login:
                self._print("[OAuth] login_session missing; retrying oauth2 auth bootstrap")
                ...
                try:
                    ...
                    self._print(f"[OAuth] /api/oauth/oauth2/auth request failed: {e}")
                    _remember_protocol_error("oauth_bootstrap", code="oauth2_auth_failed", message=str(e))

                has_login = _has_cookie("login_session")
                self._print(f"[OAuth] login_session(after fallback): {'present' if has_login else 'missing'} / cookies={_cookie_names()}")

            effective_resp = fallback_resp or primary_resp
            bootstrap_ok = bool(effective_resp) and int(getattr(effective_resp, "status_code", 0) or 0) < 400 and str(final_url or "").startswith(OAUTH_ISSUER)
            if not bootstrap_ok:
                self._print("[OAuth] bootstrap unusable; skipping authorize/continue")
        ...
        def _post_authorize_continue(referer_url: str):
            ...
            if not sentinel_authorize:
                self._print("[OAuth] authorize_continue sentinel token missing")
                _remember_protocol_error(
                    "authorize_continue_sentinel",
                    code=sentinel_error.get("code") or "sentinel_token_missing",
                    message=sentinel_error.get("message") or "Failed to build sentinel token for authorize_continue",
            ...
            try:
                ...
                self._print(f"[OAuth] authorize/continue request failed: {e}")
                _remember_protocol_error("authorize_continue", code="authorize_continue_request_failed", message=str(e))
                return None

        has_login_session, authorize_final_url, bootstrap_ok = _bootstrap_oauth_session()
        if not authorize_final_url or not bootstrap_ok:
            if bootstrap_ok is False and (self.last_protocol_error or {}).get("stage") == "oauth_bootstrap":
                return None
            if not (self.last_protocol_error or {}).get("stage"):
                _remember_protocol_error("oauth_bootstrap", code="bootstrap_url_missing", message="OAuth bootstrap did not produce a usable final URL")
            return None
        ...
        if resp_continue.status_code == 400 and "invalid_auth_step" in (resp_continue.text or ""):
            self._print("[OAuth] invalid_auth_step; retrying bootstrap via authorize")
            has_login_session, authorize_final_url, bootstrap_ok = _bootstrap_oauth_session()
            if not authorize_final_url or not bootstrap_ok:
                return None
            continue_referer = authorize_final_url if authorize_final_url.startswith(OAUTH_ISSUER) else f"{OAUTH_ISSUER}/log-in"
            ...
            if resp_continue is None:
                return None
            self._print(f"[OAuth] /authorize/continue(retry) -> {resp_continue.status_code}")

        if resp_continue.status_code != 200:
            self._print(f"[OAuth] authorize/continue failed body: {resp_continue.text[:180]}")
            try:
            ...
            _remember_protocol_error(
                ...
                message=error_message or f"authorize_continue failed with HTTP {resp_continue.status_code}",
        ...
        try:
            ...
            self._print("[OAuth] authorize/continue response could not be parsed cleanly")
            _remember_protocol_error(
                "authorize_continue",
        ...
        if not sentinel_pwd:
            self._print("[OAuth] password_verify sentinel token missing")
            _remember_protocol_error(
                "password_verify_sentinel",
                code=sentinel_pwd_error.get("code") or "sentinel_token_missing",
                message=sentinel_pwd_error.get("message") or "Failed to build sentinel token for password_verify",
        ...
        try:
            ...
            self._print(f"[OAuth] password/verify request failed: {e}")
            _remember_protocol_error("password_verify", code="password_verify_request_failed", message=str(e))
            return None

        self._print(f"[OAuth] /password/verify -> {resp_verify.status_code}")
        if resp_verify.status_code != 200:
            self._print(f"[OAuth] password/verify failed body: {resp_verify.text[:180]}")
            try:
            ...
            _remember_protocol_error(
                ...
                message=error_message or f"password_verify failed with HTTP {resp_verify.status_code}",
        ...
        try:
            ...
            self._print("[OAuth] password/verify response could not be parsed cleanly")
            _remember_protocol_error(
                "password_verify",
        ...
        if need_oauth_otp:
            self._print("[OAuth] 4/7 waiting for OAuth OTP email")
            if not mail_token:
                self._print("[OAuth] OAuth OTP requires a mail_token mailbox identifier")
                _remember_protocol_error("email_otp", code="otp_missing", message="mail_token missing for protocol OTP flow")
                return None
            ...
            while time.time() < otp_deadline and not otp_success:
                ...
                self._print(f"[OAuth] received OTP: {code}")
                try:
                    ...
                    self._print(f"[OAuth] email-otp/validate request failed: {e}")
                    _remember_protocol_error("email_otp_validate", code="email_otp_request_failed", message=str(e))
                    continue
                ...
                if resp_otp.status_code != 200:
                    ...
                    try:
                        ...
                        otp_error = {}
                    error_block = otp_error.get("error") if isinstance(otp_error.get("error"), dict) else {}
                    error_code = str(error_block.get("code") or otp_error.get("code") or "").strip()
                    error_message = str(error_block.get("message") or otp_error.get("message") or resp_otp.text[:500]).strip()
                    ...
                    self._print(f"[OAuth] OTP validation failed body: {resp_otp.text[:160]}")
                    if error_code in {"account_deactivated", "max_check_attempts"}:
                        self._print(f"[OAuth] OTP terminal error: {error_code}")
                        break
                    if error_code == "wrong_email_otp_code" or "wrong code" in error_message.lower():
                        self._print("[OAuth] wrong_email_otp_code; waiting for a new OTP...")
                    time.sleep(2)
                    continue
                ...
                try:
                    ...
                    self._print("[OAuth] email-otp/validate response parse failed")
                    _remember_protocol_error(
                        "email_otp_validate",
                ...
                self._print(f"[OAuth] OTP verified page={page_type or '-'} next={(continue_url or '-')[:140]}")
                otp_success = True
                break

            if not otp_success:
                self._print(f"[OAuth] OAuth OTP validation failed after {len(tried_codes)} attempts")
                if not (self.last_protocol_error or {}).get("stage"):
                    _remember_protocol_error(
                        "email_otp_validate",
        ...
        if not code and consent_url:
            self._print("[OAuth] 5/7 follow continue_url to resolve authorization code")
        ...
        if not code:
            self._print("[OAuth] missing authorization code")
            _remember_protocol_error("authorization_code", code="authorization_code_missing", message="Authorization code not resolved")
            return None
        ...
        if token_resp.status_code != 200:
            self._print(f"[OAuth] token request failed: {token_resp.status_code} {token_resp.text[:200]}")
            try:
            ...
            _remember_protocol_error(
                ...
                message=error_message or f"oauth/token failed with HTTP {token_resp.status_code}",
        ...
        try:
            ...
            self._print("[OAuth] token response parse failed")
            _remember_protocol_error(
                "oauth_token",
        ...
        if not data.get("access_token"):
            self._print("[OAuth] token response missing access_token")
            _remember_protocol_error("oauth_token", code="access_token_missing", message="OAuth token response missing access_token")
            return None
    ...
    for attempt in range(1, PROXY_RETRY_ATTEMPTS_PER_ACCOUNT + 1):
        ...
        try:
            ...
            if ENABLE_OAUTH and extract_codex:
                reg._print("[OAuth] extracting Codex token...")
                ...
                if oauth_ok:
                    ...
                    reg._print("[OAuth] token artifacts persisted")
                else:
                    msg = "OAuth extraction failed"
                    if OAUTH_REQUIRED:
                        reg._print(f"[OAuth] {msg} (preserving registered account with oauth=fail)")
                    else:
                        reg._print(f"[OAuth] {msg} (continuing by config)")
            elif not extract_codex:
                reg._print("[OAuth] Codex extraction disabled for this registration batch")
...
def run_batch(total_accounts: int = 3, output_file="registered_accounts.txt",
              max_workers=3, proxy=None, email_provider="mailfree", email_domain=None, extract_codex=True,
              use_proxy=True, progress_hook=None, debug=False):
    """Doc."""

    # Removed DUCKMAIL_BEARER check
    ...
    print(f"  Proxy mode: {'proxy_utils candidate pool (round-robin) with direct fallback' if use_proxy else 'direct (no proxy)'}")
    print(f"  Prefer stable proxy: {'on' if proxy_info['prefer_stable_proxy'] else 'off'} (single-proxy fallback only)")
    print(f"  Account retry budget: {PROXY_RETRY_ATTEMPTS_PER_ACCOUNT} per account")
    print(f"  Proxy validate config: {'on' if proxy_info['validate_enabled'] else 'off'} (registration now reads configured proxy candidates)")
    ...
    print(f"  OAuth: {'on' if ENABLE_OAUTH else 'off'} | required: {'yes' if OAUTH_REQUIRED else 'no'}")
    if ENABLE_OAUTH:
    ...
    with ThreadPoolExecutor(max_workers=actual_workers) as executor:
        ...
        for idx in range(1, total_accounts + 1):
            future = executor.submit(
                _register_one, idx, total_accounts, proxy, output_file, email_provider, email_domain, extract_codex, progress_hook, use_proxy, debug
            )
            futures[future] = idx
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                fail_count += 1
                with _print_lock:
                    print(f"[FAIL] worker {idx} crashed: {e}")

    elapsed = time.time() - start_time
    avg = elapsed / total_accounts if total_accounts else 0
    print(f"\n{'#'*60}")
    print(f"  Total elapsed: {elapsed:.1f}s")
    print(f"  Total: {total_accounts} | Success: {success_count} | Failed: {fail_count}")
...
def main():
    ...
    proxy_source = "config.json(proxy)" if default_fallback_proxy else (
        "env(HTTPS_PROXY/ALL_PROXY)" if env_fallback_proxy else "none"
    )

    print("[Info] Proxy mode: proxy_utils candidate pool (round-robin) with direct fallback")
    print(f"[Info] Proxy validation config retained: {'on' if PROXY_VALIDATE_ENABLED else 'off'} | target: {PROXY_VALIDATE_TEST_URL}")
    ...
    if proxy:
        ...
        print("[Info] No fallback proxy configured; direct session settings will be used")
